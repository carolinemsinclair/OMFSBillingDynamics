...
totaling<-function(inputbill){
  ...
  for (i in DOSlist){
    ...
    filler<-c("","","","","","","","","")
    billwithPayments<-rbind(billwithPayments,smallDOS,total,filler)
  }
...
roundOMFS<-function(inputbill){
  for (i in 1:nrow(inputbill)){
    ...
    inputbill[i,9]=round(as.numeric(inputbill[i,9]),digits=2)}
  }
  return(inputbill)
}
...
patient.name=strsplit(patient.name,"\n")[[1]][1]
...
titlerow<-c("","","Billing Dynamics: OMFS Report","",Sys.time(),"","","")
summarytablerow1<-c("Patient Name:", patient.name,"","Total Charges:",total.charges,"","OMFS:",OMFStotal)
...
summarytablerow4<-c("Reviewer Name:","","Insurance Balance:",insurancebalance,"","","")
summarytablerow5<-c("","","","","","","","")
summarytablerow6<-names(newbill4)
summarydf<-rbind(titlerow,summarytablerow1,summarytablerow2,summarytablerow3,summarytablerow4,summarytablerow5,
                 summarytablerow6)
